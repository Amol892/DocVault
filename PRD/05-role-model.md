...
- Removing a member or revoking a Guest's `folder_grants` row must take effect immediately on the next request — there is no separate "propagate revocation" step, because access is derived live from these rows on every check.
